...
const ff = document.getElementById('filter-button');
ff.addEventListener("click", () => {
    const filter = document.getElementById("search-filter");
    let searchValue = filter.value.toString(); // Convert the input value to a string
    console.log(searchValue);
    var htms = '';
    products.forEach(product => {
        if (product.description.includes(searchValue)) { // Check if product.name includes searchValue
